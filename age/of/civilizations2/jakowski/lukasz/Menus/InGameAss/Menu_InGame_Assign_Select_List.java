...
public class Menu_InGame_Assign_Select_List
extends Menu {
    private List<Integer> lCivs;

    public Menu_InGame_Assign_Select_List() {
        ...
        for (int i = 0; i < CFG.core.getCivsSize(); ++i) {
            if (CFG.chosenAlphabetCharachter == null) {
                ...
                menuElements.add(new Button_Classic_Classic(CFG.core.getCiv(i).getCivName() + " [" + CFG.lang.get(CFG.ideologiesMgr.getIdeologyID(CFG.core.getCiv(i).getIdeology()).getName()) + "] [" + CFG.core.getCiv(i).getCivTag() + "]", CFG.PADD, CFG.CIV_FLAG_WIDTH + CFG.PADD * 4, CFG.BUTTON_H * i + CFG.PADD * (i + 1), CFG.GAMEWIDTH - CFG.CIV_FLAG_WIDTH - CFG.PADD * 4, CFG.BUTTON_H, true));
                continue;
            }
            ...
            menuElements.add(new Button_Classic_Classic(CFG.core.getCiv(i).getCivName() + " " + CFG.lang.get(CFG.ideologiesMgr.getIdeologyID(CFG.core.getCiv(i).getIdeology()).getName()) + " [" + CFG.core.getCiv(i).getCivTag() + "]", CFG.PADD, CFG.CIV_FLAG_WIDTH + CFG.PADD * 4, CFG.BUTTON_H * j + CFG.PADD * (j + 1), CFG.GAMEWIDTH - CFG.CIV_FLAG_WIDTH - CFG.PADD * 4, CFG.BUTTON_H, true));
            ++j;
        }
