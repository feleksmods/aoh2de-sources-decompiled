...
extends Menu {
    ...
    public Menu_InGame_View_RecruitableProvincesWorst() {
        ...
        for (int a = 0; a < GameValues.gvInGame.TOP_PROVINCES_LIMIT; ++a) {
            ...
            menuElements.add(new Button_View_Recruitable(a % 2, a + 1 + ". " + (CFG.core.getProv(bestID).getName().length() > 0 ? CFG.core.getProv(bestID).getName() : CFG.core.getCiv(CFG.core.getProv(bestID).getCivId()).getCivName()), bestID, 0, tY, tempW, (CFG.SPECTATOR_MODE || CFG.core.isAlly(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), CFG.core.getProv(bestID).getCivId())) && CFG.core.getCiv(CFG.core.getProv(bestID).getCivId()).isInConstruction(bestID, ConstructionType.ARMOURY) > 0){
