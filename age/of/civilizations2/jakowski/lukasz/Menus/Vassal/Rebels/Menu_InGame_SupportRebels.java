    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            ...
            for (int i = 0; i < this.getMenuElemsSize(); ++i) {
                ...
                iSelectedCivID = this.getMenuElem(i).getCurr();
            }
            if (iSelectedCivID < 0) {
            ...
            if (GameManager.supportRebels(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), iOnCivID, iSelectedCivID, this.getMenuElem(this.getMenuElemsSize() - 3).getCurr())) {
        ...
        List<Integer> rebelsProvinces = GameManager.supportRebels_Provinces(iOnCivID, this.getMenuElem(iID).getCurr());
        this.getMenuElem(this.getMenuElemsSize() - 3).setMax(Math.max(0, Math.min((int)CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold(), GameManager.supportRebels_MaxGold(rebelsProvinces))));
        ...
        if (centerToProv && rebelsProvinces.size() > 0) {
            ...
            for (int i = 1; i < rebelsProvinces.size(); ++i) {
                if (!CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getMetProv(rebelsProvinces.get(i)) || CFG.core.getProv(rebelsProvinces.get(tBest)).getPop().getPopulationOfCivID(this.getMenuElem(iID).getCurr()) >= CFG.core.getProv(rebelsProvinces.get(i)).getPop().getPopulationOfCivID(this.getMenuElem(iID).getCurr())) continue;
                tBest = i;
            }
