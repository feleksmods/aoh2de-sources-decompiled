...
public class Menu_InGame_DisbandAllArmies
extends Menu {
    public final int getElementW2() {
        return this.getWidthM();
    }

    public Menu_InGame_DisbandAllArmies() {
        ...
        menuElements.add(new ButtonN_Pop(new Color((float)CFG.core.getCiv(iCivID).getR() / 255.0f, (float)CFG.core.getCiv(iCivID).getG() / 255.0f, (float)CFG.core.getCiv(iCivID).getB() / 255.0f, 1.0f), CFG.core.getCiv(iCivID).getCivName(), iCivID, CFG.lang.get("Army") + ": ", CFG.getNumberWthSpaces("" + CFG.core.getCiv(iCivID).getNumberOfUnits()), Images.diploArmy, CFG.COLOR_TEXT_NUM_OF_PROVINCES, 0, tY, tempWidth){
            ...
            public int getWidthE() {
                return Menu_InGame_DisbandAllArmies.this.getElementW2();
            }
        });
        menuElements.add(new Text_Desc(CFG.lang.get("DisbandAllSelectedArmiesDesc"), CFG.PADD, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 2){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_DisbandAllArmies.this.getElementW2() - CFG.PADD * 2;
            }
        });
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Close"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_DisbandAllArmies.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_DisbandAllArmies.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                IMGManager.getIMG(Images.diploArmyDisband).drawO(oSB, this.getPosXE() + this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploArmyDisband).getWidth() + CFG.PADD) / 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(Images.diploArmyDisband).getHeight() / 2 + iTranslateY);
                Renderer.drawText(oSB, this.fontID, this.getTextE(), this.getPosXE() + (this.getTextPosElem() < 0 ? this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploArmyDisband).getWidth() + CFG.PADD) / 2 + IMGManager.getIMG(Images.diploArmyDisband).getWidth() + CFG.PADD : this.getTextPosElem()) + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, this.getColorE(isActive));
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("DisbandAllSelectedArmies"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(0.33333334f, 0.41960785f, 0.18431373f, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(0.33333334f, 0.41960785f, 0.18431373f, 0.375f));
        ...
        if (iID == this.getMenuElemsSize() - 1) {
            ...
            if (TouchManager.lMABX.isEmpty()) {
                if (CFG.core.getActiveProvID() >= 0) {
                    int before = CFG.core.getProv(CFG.core.getActiveProvID()).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
                    CFG.gameAction.disbandArmy(CFG.core.getActiveProvID(), before, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
                    totalArmy += before - CFG.core.getProv(CFG.core.getActiveProvID()).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
                }
            } else {
                for (int i = TouchManager.lMABX.size() - 1; i >= 0; --i) {
                    int before = CFG.core.getProv(TouchManager.lMABX.get(i)).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
                    int prID = TouchManager.lMABX.get(i);
                    CFG.gameAction.disbandArmy(prID, before, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
                    totalArmy += before - CFG.core.getProv(prID).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
                }
                TouchManager.cMABX();
            }
            CFG.toastM.addM(CFG.lang.get("Done") + ": " + CFG.lang.get("DisbandAllSelectedArmies") + ": " + CFG.getNumberWthSpaces("" + totalArmy), CFG.COLOR_POSITIVE);
