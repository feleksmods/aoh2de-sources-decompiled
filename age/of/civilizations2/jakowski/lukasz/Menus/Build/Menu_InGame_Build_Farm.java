...
public class Menu_InGame_Build_Farm
extends Menu {
    ...
    public Menu_InGame_Build_Farm(int nProvinceID) {
        ...
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Build_Building(CFG.lang.get("BuildFarmIn") + ": ", CFG.core.getProv(this.iProvinceID).getName().length() > 0 ? CFG.core.getProv(this.iProvinceID).getName() : CFG.lang.get("Province"), Images.bFarm, BuildingsManager.getFarm_BuildCost(CFG.core.getProv(this.iProvinceID).getLvlOfFarm() + 1, this.iProvinceID), BuildingsManager.getFarm_BuildMovementCost(CFG.core.getProv(this.iProvinceID).getLvlOfFarm() + 1), 0, tY, CFG.BUTTON_W * 2){

            @Override
            public int getWidthE() {
                return Menu_InGame_Build_Farm.this.getElementW2();
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text("+" + (int)(BuildingsManager.getFarm_GrowthRateBonus(CFG.core.getProv(Menu_InGame_Build_Farm.this.iProvinceID).getLvlOfFarm() + 1) * 100.0f) + "%", CFG.COLOR_POSITIVE));
        ...
        ((MenuElemUI)menuElements.get(menuElements.size() - 1)).setMin((int)(BuildingsManager.getFarm_TechLevel(CFG.core.getProv(this.iProvinceID).getLvlOfFarm() + 1) * 100.0f));
        menuElements.add(new Text_Desc(CFG.lang.get("FarmBDesc"), CFG.PADD, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 2){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Build_Farm.this.getElementW2() - CFG.PADD * 2;
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Build_Farm.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Construct"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Build_Farm.this.getElementW() + CFG.PADD / 2;
            }
            ...
            @Override
            public void buildElemHover() {
                this.menuElemHover = Menu_InGame_Build_Farm.getHoverFarm(Menu_InGame_Build_Farm.this.iProvinceID);
            }

            @Override
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getMovemPoints() >= BuildingsManager.getFarm_BuildMovementCost(CFG.core.getProv(Menu_InGame_Build_Farm.this.iProvinceID).getLvlOfFarm() + 1) && CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold() >= (long)BuildingsManager.getFarm_BuildCost(CFG.core.getProv(Menu_InGame_Build_Farm.this.iProvinceID).getLvlOfFarm() + 1, Menu_InGame_Build_Farm.this.iProvinceID) && CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getTechLevel() >= BuildingsManager.getFarm_TechLevel(CFG.core.getProv(Menu_InGame_Build_Farm.this.iProvinceID).getLvlOfFarm() + 1);
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get(BuildingsManager.getFarm_Name(CFG.core.getProv(this.iProvinceID).getLvlOfFarm() + 1)), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                Core.drawFlagRect(oSB, Menu_InGame_Build_Farm.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Build_Farm.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.flagRect2).getHeight() / 2, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
                int imgID = Images.bFarm;
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_Build_Farm.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            if (BuildingsManager.constructFarm(this.iProvinceID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId())) {
    ...
    public static ME_Hover_v2 getHoverFarm(int provinceID) {
        ArrayList<MEHover_2E> nElements = new ArrayList<MEHover_2E>();
        ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
        ...
        nData.add(new ME_Hover_2Type_Text("" + CFG.getNumberWthSpaces("" + BuildingsManager.getFarm_BuildCost(CFG.core.getProv(provinceID).getLvlOfFarm() + 1, provinceID)), CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold() >= (long)BuildingsManager.getFarm_BuildCost(CFG.core.getProv(provinceID).getLvlOfFarm() + 1, provinceID) ? CFG.COLOR_GOLD : CFG.COLOR_NEGATIVE_2));
        ...
        nData.add(new ME_Hover_2Type_Text("" + (float)BuildingsManager.getFarm_BuildMovementCost(CFG.core.getProv(provinceID).getLvlOfFarm() + 1) / 10.0f, CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getMovemPoints() >= BuildingsManager.getFarm_BuildMovementCost(CFG.core.getProv(provinceID).getLvlOfFarm() + 1) ? CFG.COLOR_MOVEMENT : CFG.COLOR_NEGATIVE_2));
        ...
        nData.add(new ME_Hover_2Type_Image(CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getTechLevel() >= BuildingsManager.getFarm_TechLevel(CFG.core.getProv(provinceID).getLvlOfFarm() + 1) ? Images.iconTrue : Images.iconFalse, CFG.PADD, 0));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
