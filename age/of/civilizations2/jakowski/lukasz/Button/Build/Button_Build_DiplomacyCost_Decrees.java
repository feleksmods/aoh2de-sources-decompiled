...
public class Button_Build_DiplomacyCost_Decrees
extends Button_Build_DiplomacyCost {
    public String textL;
    public int iTextLW;
    public String textR;
    public int iTextRW;
    ...
    public Button_Build_DiplomacyCost_Decrees(String sText, int nImageID, int nCost, int nMovementCost, int iPosX, int iPosY, int iWidth, boolean isClickable, boolean isBuildMax, int inConstruction, float fTech, String textL, String textR, int iconIMG, Color colorRight) {
        super(sText, nImageID, nCost, nMovementCost, iPosX, iPosY, iWidth, isClickable, isBuildMax, inConstruction, fTech);
        this.textL = textL;
        this.textR = textR;
        this.iconIMG = iconIMG;
        this.colorRight = colorRight;
        CFG.glyphLay.setText(CFG.fontMain.get(this.fontID), textL);
        this.iTextLW = (int)CFG.glyphLay.width;
        CFG.glyphLay.setText(CFG.fontMain.get(this.fontID), textR);
        this.iTextRW = (int)CFG.glyphLay.width;
    }
    ...
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        ...
        Renderer.drawTextWithShadow(oSB, this.fontID, this.getTextE(), this.getPosXE() + CFG.PADD + ButtonDiplomacy.iDiploWidth + iTranslateX, this.getPosY() + this.getHeightE() / 2 - CFG.PADD / 2 - this.getTextHeight() + iTranslateY, this.getColorE(isActive));
        Renderer.drawTextWithShadow(oSB, this.fontID, this.textL, this.getPosXE() + CFG.PADD + ButtonDiplomacy.iDiploWidth + iTranslateX, this.getPosY() + this.getHeightE() / 2 + CFG.PADD / 2 + iTranslateY, this.getColorE(isActive));
        Renderer.drawTextWithShadow(oSB, this.fontID, this.textR, this.getPosXE() + CFG.PADD + ButtonDiplomacy.iDiploWidth + this.iTextLW + iTranslateX, this.getPosY() + this.getHeightE() / 2 + CFG.PADD / 2 + iTranslateY, this.colorRight);
        IMGManager.getIMG(this.iconIMG).drawO(oSB, this.getPosXE() + CFG.PADD + ButtonDiplomacy.iDiploWidth + this.iTextLW + this.iTextRW + CFG.PADD + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(this.iconIMG).getHeight() + CFG.PADD / 2 + iTranslateY, (int)((float)IMGManager.getIMG(this.iconIMG).getWidth() * this.getImageScale(this.iconIMG, ICON_SCALE)), (int)((float)IMGManager.getIMG(this.iconIMG).getHeight() * this.getImageScale(this.iconIMG, ICON_SCALE)));
    }
}
